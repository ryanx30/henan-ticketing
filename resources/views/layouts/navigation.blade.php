                        ...
                        <div class="flex items-start justify-between gap-3 px-4 py-3 border-b border-slate-100">
                            <div class="min-w-0">
                                ...
                                <button
                                    ...
                                    class="{{ $notificationCount > 0 ? '' : 'hidden' }} mt-1 whitespace-nowrap text-[11px] font-semibold text-blue-700 hover:text-blue-800 disabled:opacity-50">
                                    Mark all read
                                </button>
                            </div>
                            <span
        ...
        <div class="pt-2 pb-3 space-y-1 px-4">
            @foreach ($mobileMenus as $menu)
                ...
                <a
                    href="{{ $menu['href'] }}"
                    data-sidebar-link
                    data-sidebar-label="{{ $menu['label'] }}"
                    aria-label="{{ $menu['label'] }}"
                    class="flex items-center gap-3 rounded-md px-3 py-2 text-sm transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 {{ $mobileMenuState }}"
                    @if ($menu['active']) aria-current="page" @endif>
                    <span class="min-w-0 flex-1 truncate">{{ $menu['label'] }}</span>

                    @if ($mobileBadgeKey)
                        <span
                            data-sidebar-badge-key="{{ $mobileBadgeKey }}"
                            data-sidebar-badge-compact="0"
                            aria-hidden="true"
                            class="{{ $mobileBadgeCount > 0 ? '' : 'hidden' }} inline-flex h-5 min-w-5 shrink-0 items-center justify-center rounded-full bg-red-500 px-1.5 text-[10px] font-semibold leading-none text-white">
